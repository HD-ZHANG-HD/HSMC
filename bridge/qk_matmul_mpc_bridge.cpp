...
int main(int argc, char **argv) {
  ...
  if (!parse_args(argc, argv, args)) {
    std::cerr << "Usage: qk_matmul_mpc_bridge --party <1|2> --port <int> --address <ip> "
  ...
  std::cout << "[qk_matmul_mpc_bridge] source=he_compiler/EzPC_bolt/EzPC/SCI/tests/bert_bolt/nonlinear.cpp"
  ...
  for (int __i_stat = 0; __i_stat < args.nthreads; __i_stat++) {
    __comm_bytes += nl.iopackArr[__i_stat]->get_comm();
    __comm_rounds += nl.iopackArr[__i_stat]->get_rounds();
  }
  ...
  std::cout << "[qk_matmul_mpc_bridge] done party=" << args.party << " wrote=" << args.output_path << std::endl;
  return 0;
}
