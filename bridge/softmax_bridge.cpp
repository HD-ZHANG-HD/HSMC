...
bool parse_args(int argc, char **argv, Args &args) {
  ...
  if ((args.party != 1 && args.party != 2) || args.port <= 0 || args.dim <= 0 || args.array_size <= 0 ||
      args.input_path.empty() || args.output_path.empty() || args.l_path.empty()) {
    return false;
  }
  return true;
}
...
int main(int argc, char **argv) {
  ...
  if (!parse_args(argc, argv, args)) {
    std::cerr << "Usage: softmax_bridge --party <1|2> --port <int> --address <ip> "
                 "--nthreads <int> --ell <int> --scale <int> --dim <int> --array_size <int> "
                 "--input <path> --output <path> --l_out <path>"
              << std::endl;
    return 2;
  }

  const std::size_t total = static_cast<std::size_t>(args.dim) * static_cast<std::size_t>(args.array_size);
  std::cout << "[softmax_bridge] source=he_compiler/EzPC_bolt/EzPC/SCI/tests/bert_bolt/nonlinear.cpp"
            << " function=NonLinear::softmax(int, uint64_t*, uint64_t*, uint64_t*, int, int, int, int)"
  ...
  std::cout << "[softmax_bridge] done party=" << args.party << " wrote=" << args.output_path
            << " l_out=" << args.l_path << std::endl;
  return 0;
}
