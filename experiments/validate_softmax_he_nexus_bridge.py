...
def run_case(name: str, shape: tuple[int, ...]) -> None:
    ...
    y = run_nexus_softmax_bridge(
        x,
        NexusHeSoftmaxBridgeConfig(inverse_iterations=4, sum_scale_factor=0.01, eps=1e-8),
    )
    ref = _softmax_ref(x)
    ...
    print(f"[bridge-test] {name}: shape_ok=True mae={mae:.6f} row_sum_ok=True")
