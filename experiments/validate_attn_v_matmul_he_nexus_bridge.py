...
def run_case(name: str, b: int, s: int) -> None:
    ...
    attn = np.random.standard_normal((b, cfg.num_heads, s, s))
    out = run_nexus_attention_v_restricted_adapter(attn, qkv, cfg=cfg, return_canonical=False)
    v = _to_bhsd(qkv[2], cfg.num_heads)
    ref = (attn @ v).transpose(0, 2, 1, 3).reshape(b, s, cfg.hidden_size)
    mae = float(np.mean(np.abs(out - ref)))
    assert out.shape == (b, s, cfg.hidden_size), f"{name}: shape mismatch {out.shape}"
