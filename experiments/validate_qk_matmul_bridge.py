...
def run_case(name: str, bsz: int, heads: int, seq: int, head_dim: int, bridge: Path) -> None:
    ...
    a = q.reshape(bsz * heads, seq, head_dim)
    b = np.swapaxes(k, -1, -2).reshape(bsz * heads, head_dim, seq)
    ref = q @ np.swapaxes(k, -1, -2)

    a0, a1 = share_encode(a, ell, scale, seed=131)
    b0, b1 = share_encode(b, ell, scale, seed=137)
    ...
    for attempt in range(1, 6):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out = decode(y0, y1, ell, scale, (bsz, heads, seq, seq))
...
def main() -> None:
    bridge = Path("/home/hedong/project/he_compiler/EzPC_bolt/EzPC/SCI/build/bin/BOLT_QK_MATMUL_MPC_BRIDGE")
